...
- [ ] **Task 1: Create validation methods in TemplateService** (AC: 2, 4, 5, 6, 8)
...
- [ ] **Task 2: Integrate validation into createTemplate method** (AC: 1, 2, 4, 5, 6, 8, 9)
...
- [ ] **Task 3: Integrate validation into updateTemplate method** (AC: 1, 2, 4, 5, 6, 8, 9)
  - [ ] Modify TemplateService.updateTemplate method (from Story 2.2)
  - [ ] After updating template fields and tasks, call normalizeSequenceOrder(template.getTasks())
  - [ ] Call validateTemplate(template) before saving
  - [ ] If validation fails, ValidationException is thrown
  - [ ] If validation passes, save template with @Transactional ensuring atomic commit
  - [ ] Reference: [Source: docs/stories/2.2.story.md - Task 8]

- [ ] **Task 4: Create circular dependency detection algorithm** (AC: 6)
...
- [ ] **Task 5: Enhance ValidationException class** (AC: 8)
  - [ ] Verify `backend/src/main/java/com/magnab/employeelifecycle/exception/ValidationException.java` exists (should exist from Story 1.4b)
  - [ ] If missing, create ValidationException extending RuntimeException
  - [ ] Add constructors: ValidationException(String message), ValidationException(String message, Throwable cause)
  - [ ] Ensure GlobalExceptionHandler catches ValidationException and returns 400 Bad Request with error details
  - [ ] Reference: [Source: docs/architecture/error-handling-strategy.md]

- [ ] **Task 6: Write unit tests for validation methods** (AC: 2, 4, 5, 6, 8, 10)
...
- [ ] **Task 7: Write integration tests for validation** (AC: 2, 4, 5, 6, 8, 10)
  - [ ] Enhance `backend/src/test/java/com/magnab/employeelifecycle/controller/TemplateControllerIntegrationTest.java` (created in Story 2.2)
  - [ ] Test: POST /api/templates with zero tasks returns 400 Bad Request
  - [ ] Test: POST /api/templates with duplicate sequence_order (non-parallel) returns 400 Bad Request
  - [ ] Test: POST /api/templates with circular dependency returns 400 Bad Request
  - [ ] Test: POST /api/templates with invalid dependency_task_id returns 400 Bad Request
  - [ ] Test: POST /api/templates with valid parallel tasks (same sequence_order, isParallel=true) returns 201 Created
  - [ ] Test: PUT /api/templates/{id} with validation errors returns 400 Bad Request
  - [ ] Verify error response includes specific validation message
  - [ ] Reference: [Source: docs/architecture/test-strategy.md - Integration testing]

- [ ] **Task 8: Update Swagger documentation for validation errors** (AC: 8)
  - [ ] Update TemplateController (from Story 2.2) POST and PUT endpoint annotations
  - [ ] Add @ApiResponse(400, "Validation error - check error message for details") to POST /api/templates
  - [ ] Add @ApiResponse(400, "Validation error - check error message for details") to PUT /api/templates/{id}
  - [ ] Add example validation error responses in Swagger annotations:
  ...
  - [ ] Reference: [Source: docs/architecture/rest-api-spec.md - Error responses]
